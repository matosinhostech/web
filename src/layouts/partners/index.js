...
const useGetImagesByPartner = (partners, images = []) => partners.reduce((result, partner) => {
  const imgs = images
    .filter(({ node: { relativePath } }) => relativePath.includes(partner.slug) && relativePath.includes('dark'));

  return ({ ...result, [partner.slug]: imgs.length ? imgs[imgs.length - 1].node.publicURL : '' });
}, {});

const PartnersListLayout = (props) => {
  const {
    site: {
                ...
                <div className="content">
                  <div className="center card-invite">
                    <span className="invite-message">
                      <span>this</span><span> could be</span><span> your company</span>
                    </span>
                  </div>
